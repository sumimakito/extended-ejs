...
compile = (input, workdir = null, fileRef = null, strict = false) => {
...
render = (input, locals, fileRef = null, output = null, strict = false) => {
  const rendered = compile(input, null, fileRef, strict).render(locals);
  if (output) fs.writeFileSync(output, rendered);

  return rendered;
};

renderFile = (file, locals, output = null, strict = false) => {
  const input = fs.readFileSync(file, 'utf8');

  return render(input, locals, file, output, strict);
};
